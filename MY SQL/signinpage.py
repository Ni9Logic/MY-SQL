...
class SignInPage:
    ...
    def login(self):
        ...
        if self.user_choice == "0" or self.user_choice > "4" or len(self.user_choice) == 0:
            while self.user_choice == "0" or self.user_choice > "4" or len(self.user_choice) == 0:
                ...
                print("\t\t\t2 \u001b[1;31m-->\u001b[1;0m Sign in as user")
                print(
                    "\t\t\t3 \u001b[1;31m-->\u001b[1;0m Create a New Account")
                print("\t\t\t4 \u001b[1;31m-->\u001b[1;0m Exit")
    ...
    def userlogin(self):
        ...
        # Check's if the password is present in the database! If it's present it loads it!
        database.execute(matchpass, (self.username,))
        ...
        if check == cred:
            ...
            print(
                "\t\t\tYou have logged in successfully as\u001b[1;31m", self.username, "\u001b[1;0m")
            time.sleep(1)
        else:
            SignInPage.Logged = False
            print(
                "\t\t\tYou've entered \u001b[1;31mincorrect credentials\u001b[1;0m")
            print(
                "\t\t\tForcing session \u001b[1;31m(logged out)\u001b[1;0m\n")
